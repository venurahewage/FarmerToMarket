...
$adtel = $_SESSION['acctel'];
$ad_id = $_SESSION['adID'];
$edit = $conn->query("select * from ads where id='$ad_id'");
...
if(isset($_POST['submit'])){
	...
	if(strlen($dist)and
		...
		//$val=mysql_query("INSERT INTO ads(type,district,city,price,details) VALUES ('$type', '$dist', '$city', '$price', '$details')",$conn);

		$upd = $conn->query("UPDATE ads SET type = '$type', district = '$dist', city = '$city', details = '$details', price = '$price' WHERE id = '$ad_id'");
		$_SESSION['ltel'] = $adtel;
...
<center>
	...
	<h2>Edit Ad</h2>
	<h4>දැන්වීම වෙනස් කරන්න</h4>
	<hr class="new">
	<br>
	<form action='editad.php' method="POST">
		<?php while($toedit = $edit->fetch_assoc()){ ?>
		<input type='text' name='type' value='<?php echo $toedit['type']; ?>'>
		...
		<input type='text' name='price' value='<?php echo $toedit['price']; ?>'>
		<textarea name='details' value='<?php echo $toedit['details']; ?>'></textarea>
		<br><br>
		<hr class="new">
		<br>
		<?php } ?>
